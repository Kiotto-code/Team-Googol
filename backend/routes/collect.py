...
from database import collect_found_item, get_box_status, update_box, get_finder_by_rfid
    ...
    try:
        ...
        if box_id:
            ...
            if box_info:
                ...
                update_box(box_id, current_load=new_load)
                
                # If box is full, request collection
                if new_load >= box_info['capacity']:
                    update_box(box_id, status='collect_request')
